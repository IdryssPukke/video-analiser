...
class StreamingScraperTest(unittest.TestCase):
    def test_01_yt_cam_init(self):
        res = cam_init(url_yt)
        self.assertEqual(res.isOpened(), True)

    def test_02_agh_cam_init(self):
        res = cam_init(url_agh)
        self.assertEqual(res.isOpened(), True)

    def test_03_yt_connection(self):
        self.assertEqual(test_connection(url_yt), "passed")

    def test_04_stream_connection(self):
        self.assertEqual(test_connection(url_agh), "passed")

    def test_05_mongo_connection(self):
    ...
    def test_09_kafka_connection(self):
        producer = init_kafka('localhost:9092')
