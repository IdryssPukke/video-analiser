"""
Module to recognize cars on captured frames.
"""
import os
import time
import sys
...
def car_rec(_image, confidence=0.5, threshold=0.3):
    """
    ...
    if len(idxs) > 0:
        ...
        for i in idxs.flatten():
            ...
            if classIDs[i] == 2:
                ...
                result = car_color_classifier.predict(_image[max(y, 0):y + h, max(x, 0):x + w])
...
if __name__ == '__main__':
    ...
    for event in consumer:
        data = event.value
        img_id, timestamp, url = pickle.loads(data)
        record = collection_import.find_one({'id': img_id})
        image = pickle.loads(record['photo'])
        labels = car_rec(image)
        collection_export.insert({'id': img_id, 'timestamp': timestamp, 'url': url, "labels": labels, "comments": []})
